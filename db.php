...
function timeline($pubtime) {
    ...
    /** 超过7天 */
    return date ( 'Y-n-j H:i', $pubtime );
}
//时间线,参数是时间戳
function timeline1($time){  
    $t = time()-$time;  
    $f = array(  
        '31536000'=>'年',  
        '2592000'=>'个月',  
...
* 截取字符串
* @param 字符串 $str
* @param 截取长度 中英文字符都是1长度 $length
* @param 开始位置 $start
* @param 是否显示...默认显示 $suffix
* @param 字符编码 $charset
* 
* @return
*/
...
* @return
*/
function str_delstr($var) {
	if (is_array($var)) {
		foreach ($var as $key => $value) {
			$var[stripslashes($key)] = str_delstr($value);
		}
	} else {
...
* @return
*/
function str_addstr($var) {
	if (is_array($var)) {
		foreach ($var as $key => $value) {
			$var[addslashes($key)] = str_addstr($value);
		}
	} else {
		$var = addslashes($var);
	}
...
* 
* @param get地址 $url
* @param 读取cookie变量 $cookie
* @param 来源地址 $from
* 
* @return
*/
function get_curl($url,$cookie='',$from='',$goto=0){
	$ch=curl_init($url);
	if(!empty($from)){
...
* @param post地址 $url
* @param 字符串 $post
* @param cookie变量读取 $cookie
* @param 来源 $from
* @param 是否302跳转默认不跳转 $goto
* @param 是否保存cookie $savecookie
* 
* @return
*/
function post_curl($url,$post,$cookie='',$from='',$goto=0,$savecookie=''){
	$ch=curl_init($url);
	if(!empty($from)){
	curl_setopt($ch, CURLOPT_REFERER, $from);
	}
...
* 
* @param 接收短信手机号 $phone
* @param 短信模板内容 $content  $content ="您的手机号：{$phone}，注册验证码：{$code}，一天内提交有效！";
* @param 发送时间默认及时 2014-02-08 14:23 $time
* $phone='18291448834';
$code='3245';
$content ="您的手机号：{$phone}，注册验证码：{$code}，一天内提交有效！";
尊敬的用户，您的{变量}号会员卡已激活，请妥善保管。
* @return
*/
function sendsms($phone,$content,$time=''){
	$url = "http://api.chanyoo.cn/utf8/interface/send_sms.aspx?username=love2424&password=fjl753200&receiver={$phone}&sendtime={$time}&content=".urlencode($content);
	$xml = simplexml_load_string(get_curl($url));
	if ($xml->result >= 0)
	{
		return "发送成功!";
	}
...
//计算时间间隔
function time_day($begin_time,$end_time)//时间间隔
{
...
/*倒计时计算*/
	...
	function hidetel($phone){//隐藏手机中间四位
...
* @return
*/
function hongbao($total=1,$num=10,$min='0.01'){
	for ($i=1;$i<$num;$i++)   
{   
...
$arr[] = array('i'=>$num,'money'=>$total,'total'=>0);
   return $arr;  
}